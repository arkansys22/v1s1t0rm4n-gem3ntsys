...
class Paneluser extends CI_Controller {
	...
	public function verify_visitor()
	{
		if ($this->session->level=='1'){
			cek_session_developer($this->session->id_session);
			$data['record'] = $this->Crud_m->view_join_where_ordering_user4('user','user_detail','id_session','id_session',array('level'=>'4','user_detail_vdatefrom'=>date('20y-m-d')),'user_detail_vdatefrom','DESC');
